...
const categories = ["Popular", "Transportation", "Home Help", "Errands", "Presence", "Lifestyle", "Business"];
...
export default function NewRequestPage() {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-5 lg:py-8 max-w-[1500px] mx-auto">
      ...
      {/* Stepper */}
      <div className="mb-6 lg:mb-8 flex items-center gap-2 sm:gap-4 overflow-x-auto scrollbar-none -mx-4 px-4">
      ...
      <div className="grid lg:grid-cols-[1fr_360px] xl:grid-cols-[1fr_400px] gap-6">
        <section>
          ...
          <div className="mt-4 flex gap-2 overflow-x-auto scrollbar-none -mx-4 px-4 pb-1">
            {categories.map((c, i) => (
              <button
                key={c}
                className={cn(
                  "shrink-0 inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-semibold transition border",
                  i === 0 ? "bg-brand-50 text-brand-700 border-brand-200" : "bg-white text-slate-700 border-slate-200 hover:bg-slate-50"
                )}
              >
          ...
          {/* Service grid — 2-col mobile, 4-col sm+ */}
            ...
            {popular.map((p) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "relative text-left rounded-2xl border p-3 sm:p-4 transition min-h-[170px] sm:min-h-[200px] flex flex-col",
                    isSel ? "border-brand-500 bg-brand-50/40 ring-2 ring-brand-200" : "border-slate-100 bg-white hover:border-slate-300"
                  )}
                >
                  {isSel && (
                  ...
                  <span className={`inline-flex w-10 h-10 sm:w-12 sm:h-12 rounded-xl items-center justify-center mb-2 sm:mb-3 ${p.tone}`}>
                    <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
                  </span>
                  <div className="text-sm font-bold text-slate-900">{p.title}</div>
                  <div className="text-[11px] sm:text-xs text-slate-500 mt-1 line-clamp-2 sm:line-clamp-3">{p.blurb}</div>
                  <div className="text-xs font-semibold text-brand-700 mt-2 sm:mt-3 mt-auto pt-2 inline-flex items-center gap-1">
                    {p.from ? `From $${p.from}` : <>View all services <ArrowRight className="w-3 h-3" /></>}
                  </div>
                </button>
              );
            })}
          ...
          <div className="mt-7 lg:mt-8">
            ...
            <div className="mt-3 rounded-2xl bg-white border border-slate-100 p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              <span className="inline-flex w-11 h-11 rounded-xl bg-brand-50 items-center justify-center shrink-0">
                <Sparkles className="w-5 h-5 text-brand-600" />
              </span>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-bold text-slate-900">Custom Request</div>
                <div className="text-xs text-slate-500 mt-1">Describe what you need and we'll match you with the right person for the job.</div>
              </div>
              <button className="text-sm font-semibold text-brand-700 bg-white border border-brand-200 hover:bg-brand-50 px-4 py-2.5 rounded-xl shrink-0 w-full sm:w-auto">
                Create Custom Request
              </button>
            </div>
          </div>

          {/* Trust strip — 2x2 mobile, 4-col sm+ */}
          <div className="mt-5 rounded-2xl bg-brand-50/50 border border-brand-100 p-4 sm:p-5 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
            {[
              ...
              { icon: User, title: "24/7 Support", sub: "Our support team is available anytime you need help." },
            ].map((t) => (
              <div key={t.title} className="flex items-start gap-2.5">
                ...
                <div className="min-w-0">
                  <div className="text-xs font-bold text-slate-900 leading-tight">{t.title}</div>
                  <div className="text-[11px] text-slate-500 mt-0.5 leading-snug">{t.sub}</div>
                </div>
              </div>
            ))}
          </div>

          {/* MOBILE-ONLY: Summary + How it works + Continue (inline flow, no side panel on mobile) */}
...
function Dash() {
  return <span className="hidden sm:block flex-1 max-w-[80px] h-px border-t border-dashed border-slate-300" />;
}
    ...
    <div className="rounded-2xl bg-white border border-slate-100 p-4 sm:p-5">
      ...
      <dl className="text-sm divide-y divide-slate-100">
        ...
        <div className="py-3 flex items-center justify-between">
          ...
          <dd className="font-semibold text-slate-900">30 - 60 min</dd>
        </div>
...
function HowItWorks() {
  ...
  return (
    <div className="rounded-2xl bg-white border border-slate-100 p-4 sm:p-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-1 gap-4 sm:gap-3">
        {steps.map((s) => (
          <div key={s.n} className="flex flex-col lg:flex-row items-start gap-2 lg:gap-3">
            <span className="w-6 h-6 rounded-full bg-brand-100 text-brand-700 text-[11px] font-bold flex items-center justify-center shrink-0">{s.n}</span>
            <div className="min-w-0">
              <div className="text-xs sm:text-sm font-bold text-slate-900 leading-tight">{s.title}</div>
              <div className="text-[11px] text-slate-500 mt-0.5 leading-snug">{s.sub}</div>
            </div>
          </div>
        ))}
      </div>
